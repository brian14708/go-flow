...
func Generate(opt GenerateOptions) ([]byte, error) {
	...
	for _, entry := range types {
		fields, err := makeFields(entry.typename)
		if err != nil {
			return nil, err
		}
...
func GenerateTest(opt GenerateOptions) ([]byte, error) {
	...
	for _, entry := range types {
		f.Func().Id("Test" + caser.String(entry.name)).Params(
...
func makeFuncCaller(funcs []caller, v Code) (Code, error) {
	callBlks := make([]Code, 0, len(funcs))
...
func makeFields(typename Code) (Dict, error) {
